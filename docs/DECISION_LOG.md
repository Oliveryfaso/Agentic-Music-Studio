...
> 状态：已批准
> 初始决策日期：2026-08-11；最新增补：2026-09-24
...
> 历史决定：视觉配色与表现形式已由 2026-09-24 的 ADR-018 替代。这里保留原始决策，不再作为当前配色要求。
...
## ADR-017：S7 作为个人作品集封板门

**决定**：S7 以可演示、可解释、可审计的个人作品集首版封板，不把多租户 SaaS、长时负载、完整 OTel/告警或全故障排列强行加入完成定义。封板表面固定为权威 Export、只读 Run Inspector、About/Eval 页面、至少 96 条内部 Eval 和一个精确可复现的本地阶段门。

**证据规则**：公开报告必须把 measured、expected reject 与 not measured 分开；历史付费样本不能冒充本轮调用，聚焦测试的延迟桶不能冒充生产 P95。Export/Inspector 只读权威 PostgreSQL/Artifact 事实，不因页面访问创建 Job、Revision 或 Graph 副作用。

**后续触发器**：只有准备公开托管、多人共享、长时间运行，或出现数据/权限/费用/恢复问题时，才为对应风险建立新计划。未触发前优先保持作品集可运行、文档诚实和 Agent/LangGraph 学习价值，不为测试数量本身继续扩建平台。

## ADR-018：明亮音乐工作台与产品化表达

**决定**：根据用户对系统性前端升级的授权，抛弃原深色科幻外观，采用雾白画布、白色面板、深青绿主操作和柔和音轨类别色。首页聚焦建立/恢复作品；Brief、审批、候选、Studio、导出解释当前状态和下一步；ID、来源与任务证据使用渐进展示，不从 Inspector 移除。

**范围**：调整用户可见页面、交互可达性、Canvas 配色与文档。唯一 Parent Graph、API DTO、模型预算、HITL、不可变 Revision、音频与持久恢复合同不变，不以界面简洁为由跳过审批或掩盖错误。Studio 明示保存版本试听与草稿的差别，手机保留查看/试听/审批边界。

**作品集表达**：README 按产品定位、截图、快速开始、创作流程、Agent 设计、证据与限制组织。演示截图使用真实前端和隔离示例数据，并明确标注非实测生成结果；历史付费样本、规则 Eval 与未验证项分别说明。开发与 LangSmith 配置独立归档，避免旧阶段状态与当前产品冲突。

**验证**：此呈现层切片以测试驱动的交互检查、前端测试/构建及桌面/手机浏览器检查验收；不因未改动的 Graph/Worker 重跑全阶段故障矩阵或产生模型费用。它不构成新一轮真实音乐生成或生产负载验收。
