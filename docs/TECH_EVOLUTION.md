...
## 2026-08-20：S4 四个 Style Pack 与确定性 Theory Engine

- 新增四个严格且不可变的 `StylePack v1`：Synth Ambient、Minimal Electronic、Classical Chamber、Jazz Harmony & Improvisation。每包携带版本、曲式、四个导出角色/音域、和声/节奏/音色约束、avoidance、ProductionRecipe、SymbolicExemplar、Project-authored reviewed source/license snapshot 与 Lite Preset Palette；未知、未审核或不兼容知识 fail closed。
- 新增纯领域 `TheoryEngine v1`，只读取权威 ArrangementIR Note/Track/Section facts，输出稳定 rule ID、error/warning/advice、bar/track evidence、explanation code 和有界建议。缺角色/越音域是阻塞错误；Minimal bass/drum onset lock、Classical parallel perfect-motion review、Jazz strong-beat guide tone 与 avoid-note 都报告实际测量计数，不让 RAG/模型文本决定音符合法性。
- `MusicStrategyRouter` 在同一 Plan/Revision 命令链内选择四种确定性编译策略；Minimal 使用短音型、bass/drum lock 与 syncopation，Classical 使用分声部 stepwise contour，Jazz 使用成组 chord attack、80-tick swing 和 strong-beat guide tone。所有输出继续经 EditorCommand/apply_commands 形成 ArrangementIR，不允许模型或策略直接写 Revision。
