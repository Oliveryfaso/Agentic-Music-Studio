...
__all__ = [
    "CompositionBrief", "CompositionPlan", "build_composition_plan_graph",
    "build_parent_graph", "initial_import_state", "initial_plan_state",
...
def __getattr__(name: str):  # type: ignore[no-untyped-def]
    ...
    if name in {"build_composition_plan_graph", "initial_plan_state"}:
        ...
        return {"build_composition_plan_graph": build_composition_plan_graph, "initial_plan_state": initial_plan_state}[name]
    if name in {"build_parent_graph", "initial_import_state", "initial_time_stretch_state"}:
        from motif_forge.agent.parent_graph import build_parent_graph, initial_import_state, initial_time_stretch_state
        return {"build_parent_graph": build_parent_graph, "initial_import_state": initial_import_state, "initial_time_stretch_state": initial_time_stretch_state}[name]
