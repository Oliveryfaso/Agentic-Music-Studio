...
export function StudioPage({ projectId, revisionId }: { projectId: string; revisionId: string }) {
  ...
  const projection = useMemo(() => studio.data ? projectTimeline(studio.data.arrangement_ir) : null, [studio.data]);
  ...
  if (!studio.data || !project.data || !projection) return null;

  const rootReady = project.data.storage_root_status === "ready";
  return (
    <section className="studio-page" aria-labelledby="studio-title">
      <header className="studio-hero">
        <div><p className="eyebrow">READ-ONLY ARRANGEMENT / REVISION TRUTH</p><h1 id="studio-title">{project.data.name} / Revision</h1><p>试听已持久化的完整作品，并检查 ArrangementIR 的结构与轨道。编辑能力将在 S6 接入。</p></div>
      ...
      {projection.tracks.length === 0 ? (
        ...
        <section className="studio-panel arrangement-panel" aria-labelledby="arrangement-title">
          <div className="panel-heading"><div><p className="eyebrow">ARRANGEMENT IR / PPQ {studio.data.arrangement_ir.ppq}</p><h2 id="arrangement-title">只读时间线</h2></div><span className="status-pill available">持久化</span></div>
          <div className="arrangement-workspace"><TrackHeaders tracks={projection.tracks} /><ArrangementTimeline projection={projection} currentTime={transport.currentTime} /></div>
