...
export function RunInspectorPage({ runId }: { runId: string }) {
  ...
  return <section className="inspection-page">
    ...
    <section className="inspection-panel"><div className="panel-heading"><div><p className="eyebrow">PERSISTED EVENT ORDER</p><h2>Graph Timeline</h2></div><span className="count-badge">{value.timeline.length}</span></div>{value.timeline_truncated && <StatusBanner tone="warning" message="时间线已截断" detail="只显示最近 200 条持久事件。" />}<div className="inspection-table-scroll"><table><thead><tr><th>Seq</th><th>Phase</th><th>Event</th><th>Safe summary</th><th>Time</th></tr></thead><tbody>{value.timeline.map((event) => <tr data-testid="inspection-event" data-sequence={event.sequence} key={event.sequence}><td>{event.sequence}</td><td>{event.phase}</td><td>{event.event_type}</td><td><code>{formatSummary(event.summary)}</code></td><td>{formatTime(event.created_at)}</td></tr>)}</tbody></table></div></section>
