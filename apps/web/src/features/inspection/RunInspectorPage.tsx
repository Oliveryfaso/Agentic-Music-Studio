...
export function RunInspectorPage({ runId }: { runId: string }) {
  ...
  if (query.isPending) return <section className="loading-state"><div className="spectral-loader" aria-hidden="true"><i /><i /><i /><i /><i /></div><h2>正在读取运行记录</h2><p>整理执行节点、人工决策、模型用量和交付结果。</p></section>;
  if (query.isError) return <section className="error-state" role="alert"><span>!</span><div><h2>无法读取运行记录</h2><p>{message(query.error)}</p><button className="secondary-inline" type="button" onClick={() => void query.refetch()}>重试</button></div></section>;
  const value = query.data;
  return <section className="inspection-page">
    <header className="inspection-hero"><div><p className="eyebrow">BEHIND THE MUSIC</p><h1>运行检查 · Inspector</h1><p>看清 Agent 经过了哪些步骤，在哪里等待你的决定，以及最终交付了什么。</p></div><div className="inspection-links"><a className="secondary-inline" href={routePath({ name: "run", runId })}>返回创作进度</a>{value.run.revision_id && <><a className="secondary-inline" href={routePath({ name: "studio", projectId: value.run.project_id, revisionId: value.run.revision_id })}>打开 Studio</a><a className="secondary-inline" href={routePath({ name: "export", projectId: value.run.project_id, revisionId: value.run.revision_id })}>查看导出</a></>}</div></header>
    {value.run.error_code && <StatusBanner tone="danger" message="这次任务遇到了错误" detail={value.run.error_code} />}
